...
class GRInteger : Token {
    ...
    override func parse(input:String) -> String? {
        ...
        if let strVal = self.stringValue {
            ...
            self.calculatedValue.assign(int: Int(strVal)!)
        }
        return returnValue
    }
}
...
/// A Token subclass for parsing strings only containing uppercase letters
...
/// A Token subclass for parsing strings without quotation marks
